...
<head>
	...
	<meta name="description" content="Page pour chercher des stages." />
    <!-- CSS -->
	...
	<title>Pilote - Chercher stage</title>
</head>
<body>
<header>
   ...
   <?php
                ...
                try{
                    ...
                    if(!isset($_GET['s-bar']) || $_GET['s-bar']==NULL){
                        ...
                        ON q.id_intership = n.id_intership) as l 
                        ON x.id_intership = l.id_intership
                        WHERE email_comp IS NOT NULL AND (b LIKE '%" . $_GET['s-bar'] . "%' OR h LIKE '%" . $_GET['s-bar'] . "%' OR g LIKE '%" . $_GET['s-bar'] . "%' 
                        OR d LIKE '%" . $_GET['s-bar'] . "%' OR name_comp LIKE '" . $_GET['s-bar'] . "' OR intership_term LIKE '" . $_GET['s-bar'] . "' 
                        OR date_offer LIKE '" . $_GET['s-bar'] . "' OR num_place LIKE '" . $_GET['s-bar'] . "');";
                    }
                    $recipesStatement = $conn->prepare($sqlREQ);
                    $recipesStatement->execute();
                    $recipes = $recipesStatement->fetchAll();
